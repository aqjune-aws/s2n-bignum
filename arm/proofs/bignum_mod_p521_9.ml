...
let BIGNUM_MOD_P521_9_SUBROUTINE_SAFE = time prove
 (`exists f_events. !z x e pc returnaddress.
      nonoverlapping (word pc,0x84) (z,8 * 9)
      ==> ensures2 arm
           (\(s1,s2).
                aligned_bytes_loaded s1 (word pc) bignum_mod_p521_9_mc /\
                read PC s1 = word pc /\
                aligned_bytes_loaded s2 (word pc) bignum_mod_p521_9_mc /\
                read PC s2 = word pc /\
  ...
  ARM_N_STUTTER_LEFT_TAC BIGNUM_MOD_P521_9_EXEC (1--33) None THEN
  ARM_N_STUTTER_RIGHT_TAC BIGNUM_MOD_P521_9_EXEC (1--33) "'" None THEN
