...
let modstep384_tac regs topw n =
  ...
  MP_TAC THENL
   [MATCH_MP_TAC MASK_AND_VALUE_FROM_CARRY_LT THEN
    ...
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN EXPAND_TAC "a" THEN
  ...
  MP_TAC THENL
   [CONV_TAC SYM_CONV THEN MATCH_MP_TAC MOD_UNIQ_BALANCED_REAL THEN
    ...
    COND_CASES_TAC THEN ASM_REWRITE_TAC[BITVAL_CLAUSES] THEN
    CONV_TAC WORD_REDUCE_WEAK_CONV THEN REWRITE_TAC[p_384] THEN
   ...
   [EXPAND_TAC "a1" THEN REWRITE_TAC[bignum_of_wordlist] THEN
    ...
    CONJ_TAC THENL
     [ASM_REWRITE_TAC[REAL_SUB_LE; REAL_OF_NUM_CLAUSES] THEN
      UNDISCH_TAC `a < 2 EXP (64 * 6)` THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
