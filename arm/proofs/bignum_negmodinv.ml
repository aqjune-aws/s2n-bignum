...
let numsteps = count_nsteps (concl BIGNUM_NEGMODINV_SUBROUTINE_CORRECT)
    BIGNUM_NEGMODINV_EXEC;;

let full_spec = mk_safety_spec
    ~numinstsopt:numsteps
    (assoc "bignum_negmodinv" subroutine_signatures)
    BIGNUM_NEGMODINV_SUBROUTINE_CORRECT
    BIGNUM_NEGMODINV_EXEC;;

let BIGNUM_NEGMODINV_SUBROUTINE_SAFE = time prove
 (full_spec,
  PROVE_SAFETY_SPEC BIGNUM_NEGMODINV_EXEC);;
