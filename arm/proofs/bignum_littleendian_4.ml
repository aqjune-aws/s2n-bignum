...
let BIGNUM_LITTLEENDIAN_4_SUBROUTINE_SAFE = time prove
 (`exists f_events. !z x e pc returnaddress.
      ...
      ==> ensures2 arm
           (\(s1,s2).
                aligned_bytes_loaded s1 (word pc) bignum_littleendian_4_mc /\
                read PC s1 = word pc /\
                aligned_bytes_loaded s2 (word pc) bignum_littleendian_4_mc /\
                read PC s2 = word pc /\
  ...
  ARM_N_STUTTER_LEFT_TAC BIGNUM_LITTLEENDIAN_4_EXEC (1--69) None THEN
  ARM_N_STUTTER_RIGHT_TAC BIGNUM_LITTLEENDIAN_4_EXEC (1--69) "'" None THEN
