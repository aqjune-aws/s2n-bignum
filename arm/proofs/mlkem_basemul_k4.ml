...
(*
let numsteps = count_nsteps (concl MLKEM_BASEMUL_K4_SUBROUTINE_CORRECT)
...
let MLKEM_BASEMUL_K4_SUBROUTINE_SAFE = time prove
 ...
 forall srcA srcB srcBt dst x0 y0 y0t x1 y1 y1t x2 y2 y2t x3 y3 y3t pc stackpointer returnaddress.
     ...
     ==> ensures2 arm
         (\(s1,s2).
              aligned_bytes_loaded s1 (word pc) mlkem_basemul_k4_mc /\
              read PC s1 = word pc /\
              aligned_bytes_loaded s2 (word pc) mlkem_basemul_k4_mc /\
              read PC s2 = word pc /\
