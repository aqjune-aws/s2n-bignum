...
let PROVE_IT = REWRITE_TAC[bignum_of_wordlist] THEN
    REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
...
let BIGNUM_EMONTREDC_8N_NEON_CORRECT = time prove
  ...
  ASM_CASES_TAC `k4 = 0` THENL
   [UNDISCH_THEN `k4 = 0` SUBST_ALL_TAC THEN
    ...
    ASM_REWRITE_TAC[VAL_WORD_USHR; NUM_REDUCE_WEAK_CONV `2 EXP 2`; ARITH_RULE `0 < 1`] THEN
    DISCH_TAC THEN
    ARM_STEPS_TAC BIGNUM_EMONTREDC_8N_NEON_EXEC [861] THEN
    ENSURES_FINAL_STATE_TAC THEN
    UNDISCH_TAC `8 divides k` THEN
    ASM_REWRITE_TAC[VAL_WORD_USHR; NUM_REDUCE_WEAK_CONV `2 EXP 2`;
  ...
  CONJ_TAC THENL
   [ARM_SIM_TAC BIGNUM_EMONTREDC_8N_NEON_EXEC (1--5) THEN
    ASM_REWRITE_TAC[VAL_WORD_USHR; NUM_REDUCE_WEAK_CONV `2 EXP 2`] THEN
    ...
    ASM_REWRITE_TAC[word_ushr; NUM_REDUCE_WEAK_CONV `2 EXP 2`] THEN
  ...
  [ REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
    ...
    REWRITE_TAC[GSYM BIGNUM_FROM_MEMORY_BYTES] THEN
    REWRITE_TAC[NUM_REDUCE_WEAK_CONV `8 * 4`] THEN
    REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
    DISCH_TAC THEN
    SUBGOAL_THEN
     `(!i. i < 4
   ...
   [ALL_TAC;
    ...
    SUBGOAL_THEN `cout = bitval bout` SUBST_ALL_TAC THENL
     [EXPAND_TAC "bout" THEN UNDISCH_TAC `cout < 2` THEN
      ...
      CONV_TAC WORD_REDUCE_WEAK_CONV THEN REWRITE_TAC[BITVAL_CLAUSES];
      ALL_TAC] THEN
    SUBGOAL_THEN
     `bitval
       (2 EXP 64 <=
    ...
    SUBST_ALL_TAC THENL
     [POP_ASSUM_LIST(K ALL_TAC) THEN AP_TERM_TAC THEN
      ...
      REWRITE_TAC[BITVAL_CLAUSES] THEN CONV_TAC WORD_REDUCE_WEAK_CONV THEN
  ...
  ASM_CASES_TAC `k4 = 1` THENL
   [UNDISCH_THEN `k4 = 1` SUBST_ALL_TAC THEN
    ...
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN CONV_TAC WORD_RULE;
    ALL_TAC] THEN
  ...
  ASM_CASES_TAC `k4 = 2` THENL [
    ...
    SUBGOAL_THEN
      ...
      CONV_TAC (ONCE_DEPTH_CONV NUM_REDUCE_WEAK_CONV) THEN REWRITE_TAC[WORD_ADD_0] THEN
      FAIL_TAC "unreachable";
  ...
  ENSURES_SEQUENCE_TAC `pc + 0x82c` (apply_i inner_loop_invariant `2:num`)
    ...
    SUBGOAL_THEN `!j. j < 4
      ...
      CONV_TAC (LAND_CONV (ONCE_DEPTH_CONV NUM_REDUCE_WEAK_CONV)) THEN
      STRIP_TAC] THEN
