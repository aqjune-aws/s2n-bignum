...
let numsteps = count_nsteps (concl BIGNUM_ADD_SUBROUTINE_CORRECT)
    BIGNUM_ADD_EXEC;;

let full_spec = mk_safety_spec
    ~numinstsopt:numsteps
    (assoc "bignum_add" subroutine_signatures)
    BIGNUM_ADD_SUBROUTINE_CORRECT
    BIGNUM_ADD_EXEC;;

let BIGNUM_ADD_SUBROUTINE_SAFE = time prove
 (full_spec,
  PROVE_SAFETY_SPEC BIGNUM_ADD_EXEC);;
