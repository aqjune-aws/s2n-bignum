...
(* by stating the property for each basic block in relational Hoare triple   *)
(* ========================================================================= *)
...
let straight_line_tac (n:int):tactic =
  ...
  REPEAT_N (if n == 0 then 0 else 1) (
    ...
    ARM_N_STUTTER_LEFT_TAC COPY_EXEC (1--n) None THEN
    ARM_N_STUTTER_RIGHT_TAC COPY_EXEC (1--n) "'" None) THEN
...
let INIT_CONST_TIME = prove(
  `forall pc k z n x.
    ...
    ==> ensures2 arm
      (\(s1,s2). aligned_bytes_loaded s1 (word pc) bignum_copy_mc /\ read PC s1 = word pc /\
                 aligned_bytes_loaded s2 (word pc) bignum_copy_mc /\ read PC s2 = word pc /\
                 C_ARGUMENTS [word k; z; word n; x] s1 /\
                 C_ARGUMENTS [word k; z; word n; x] s2 /\
                 read events s1 = read events s2)
      (\(s1,s2). aligned_bytes_loaded s1 (word pc) bignum_copy_mc /\
                 (~(0 = MIN k n) /\ read PC s1 = word (pc + 0x10) \/ 0 = MIN k n /\ read PC s1 = word (pc + 0x24)) /\
                 aligned_bytes_loaded s2 (word pc) bignum_copy_mc /\
                 (~(0 = MIN k n) /\ read PC s2 = word (pc + 0x10) \/ 0 = MIN k n /\ read PC s2 = word (pc + 0x24)) /\
...
let COPYLOOP_CONST_TIME = prove(
  `forall pc k z n x.
    ...
    ==> ensures2 arm
      (\(s1,s2). aligned_bytes_loaded s1 (word pc) bignum_copy_mc /\ read PC s1 = word (pc + 0x10) /\
                 aligned_bytes_loaded s2 (word pc) bignum_copy_mc /\ read PC s2 = word (pc + 0x10) /\
                 C_ARGUMENTS [word k; z; word (MIN k n); x; word 0] s1 /\
                 C_ARGUMENTS [word k; z; word (MIN k n); x; word 0] s2 /\
                 read events s1 = read events s2)
      (\(s1,s2). aligned_bytes_loaded s1 (word pc) bignum_copy_mc /\ read PC s1 = word (pc + 0x24) /\
                 aligned_bytes_loaded s2 (word pc) bignum_copy_mc /\ read PC s2 = word (pc + 0x24) /\
  ...
  REPEAT STRIP_TAC THEN
  ENSURES2_WHILE_PAUP_TAC `0:num` `MIN k n` `pc + 0x10` `pc + 0x20` `pc + 0x10` `pc + 0x20`
  `\i s1 s2.
  ...
  `forall pc k z n x.
    ...
    ==> ensures2 arm
      (\(s1,s2). aligned_bytes_loaded s1 (word pc) bignum_copy_mc /\ read PC s1 = word (pc + 0x24) /\
                 ...
                 read events s1 = read events s2)
      (\(s1,s2). aligned_bytes_loaded s1 (word pc) bignum_copy_mc /\
                 (~(k <= MIN k n) /\ read PC s1 = word (pc + 0x2c) \/ k <= MIN k n /\ read PC s1 = word (pc + 0x3c)) /\
                 aligned_bytes_loaded s2 (word pc) bignum_copy_mc /\
                 (~(k <= MIN k n) /\ read PC s2 = word (pc + 0x2c) \/ k <= MIN k n /\ read PC s2 = word (pc + 0x3c)) /\
                 C_ARGUMENTS [word k; z; word (MIN k n); x; word (MIN k n)] s1 /\
                 C_ARGUMENTS [word k; z; word (MIN k n); x; word (MIN k n)] s2 /\
                 read events s1 = read events s2)
      (\s s'. T)
...
let PADLOOP_CONST_TIME = prove(
  `forall pc k z n x.
    ...
    nonoverlapping (word pc, 0x40) (z, 8 * k) /\ nonoverlapping (word pc, 0x40) (z, 8 * k)
    ==> ensures2 arm
      (\(s1,s2). aligned_bytes_loaded s1 (word pc) bignum_copy_mc /\ read PC s1 = word (pc + 0x2c) /\
                 aligned_bytes_loaded s2 (word pc) bignum_copy_mc /\ read PC s2 = word (pc + 0x2c) /\
                 C_ARGUMENTS [word k; z; word (MIN k n); x; word (MIN k n)] s1 /\
                 C_ARGUMENTS [word k; z; word (MIN k n); x; word (MIN k n)] s2 /\
                 read events s1 = read events s2)
      (\(s1,s2). aligned_bytes_loaded s1 (word pc) bignum_copy_mc /\ read PC s1 = word (pc + 0x3c) /\
                 aligned_bytes_loaded s2 (word pc) bignum_copy_mc /\ read PC s2 = word (pc + 0x3c) /\
  ...
  REWRITE_TAC [C_ARGUMENTS; NONOVERLAPPING_CLAUSES] THEN
  REPEAT STRIP_TAC THEN
  ENSURES2_WHILE_PAUP_TAC `(MIN k n):num` `k:num` `pc + 0x2c` `pc + 0x38` `pc + 0x2c` `pc + 0x38`
  `\i s1 s2.
      read X0 s1 = word k /\ read X1 s1 = z /\ read X2 s1 = word (MIN k n) /\ read X3 s1 = x /\ read X4 s1 = word i /\
      read X0 s2 = word k /\ read X1 s2 = z /\ read X2 s2 = word (MIN k n) /\ read X3 s2 = x /\ read X4 s2 = word i /\
      read events s1 = read events s2`
