 ...
 (`!z x n pc returnaddress.
  ...
  ASSUME_TAC THENL
   [REWRITE_TAC[BIGNUM_OF_WORDLIST_SPLIT_RULE(4,1)] THEN
    ...
    SUBST1_TAC(SYM(NUM_REDUCE_WEAK_CONV `2 EXP 52 - 1`)) THEN
  ...
  SUBGOAL_THEN `val(htop:int64) < 2 EXP 56` ASSUME_TAC THENL
   [EXPAND_TAC "htop" THEN REWRITE_TAC[VAL_WORD_ADD] THEN
    ...
    REWRITE_TAC[DIMINDEX_64; SYM(NUM_REDUCE_WEAK_CONV `2 EXP 9 - 1`)] THEN
...
let LOCAL_MUL_P521_CORRECT = prove
 (`!z x y a b pc stackpointer returnaddress.
  ...
  CONV_TAC NUM_REDUCE_WEAK_CONV THEN CONV_TAC(ONCE_DEPTH_CONV EXPAND_NSUM_CONV) THEN
  CONV_TAC NUM_REDUCE_WEAK_CONV THEN ASM_REWRITE_TAC[BITVAL_CLAUSES] THEN
  CONV_TAC(LAND_CONV(RAND_CONV(RAND_CONV CONJ_CANON_CONV))) THEN
  ASM_REWRITE_TAC[] THEN REWRITE_TAC[p_521] THEN
  CONV_TAC NUM_REDUCE_WEAK_CONV THEN REWRITE_TAC[REAL_CONGRUENCE] THEN
  CONV_TAC NUM_REDUCE_WEAK_CONV THEN REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES] THEN
  REAL_INTEGER_TAC);;
