 ...
 (*** Case split to trivialize the "no terms in sum" case ***)
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    REWRITE_TAC[WORD_REDUCE_WEAK_CONV `word 18446744073709551616:int64`] THEN
    CONV_TAC WORD_RULE;
  ...
  GHOST_INTRO_TAC `l':num` `\s. val(read X6 s)` THEN
  REWRITE_TAC[VAL_WORD_GALOIS; DIMINDEX_64] THEN
  REWRITE_TAC[NUM_REDUCE_WEAK_CONV `2 EXP 64 - 8`] THEN
  REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
  ...
  ASSUME_TAC THENL
   [CONV_TAC NUM_REDUCE_WEAK_CONV THEN
    ...
    SUBST1_TAC(SYM(WORD_REDUCE_WEAK_CONV `word_neg(word 8):int64`)) THEN
    CONV_TAC WORD_RULE;
    ALL_TAC] THEN
