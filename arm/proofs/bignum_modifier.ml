   ...
   [GHOST_INTRO_TAC `n:num` `bignum_from_memory (t,k)` THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM_TAC BIGNUM_MODIFIER_EXEC (1--6) THEN
      ...
      SUBST1_TAC(SYM(WORD_REDUCE_WEAK_CONV `word_not(word 0):int64`)) THEN
      ONCE_REWRITE_TAC[GSYM COND_SWAP] THEN REWRITE_TAC[GSYM WORD_MASK] THEN
      SUBST1_TAC(ARITH_RULE `63 = 2 EXP 6 - 1`) THEN
      REWRITE_TAC[VAL_WORD_AND_MASK_WORD] THEN
      CONV_TAC NUM_REDUCE_WEAK_CONV THEN REWRITE_TAC[MOD_64_CLAUSES] THEN
     ...
     [SUBGOAL_THEN `64 * k = c MOD 64 + (64 * k - c MOD 64)` SUBST1_TAC THENL
      ...
      ASM_CASES_TAC `bigdigit n (k - 1) = 0` THENL
       [ASM_REWRITE_TAC[BITSIZE_0; MULT_CLAUSES; ADD_CLAUSES] THEN
        CONV_TAC NUM_REDUCE_WEAK_CONV THEN REWRITE_TAC[BITSIZE_LE; SUB_0] THEN
   ...
   [GHOST_INTRO_TAC `rhi:num` `\s. val(read X9 s)` THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM_TAC BIGNUM_MODIFIER_EXEC (1--4) THEN
      ...
      CONV_TAC REAL_RAT_REDUCE_WEAK_CONV THEN REWRITE_TAC[WORD_MASK] THEN
      EXPAND_TAC "b" THEN REWRITE_TAC[GSYM NOT_LT; COND_SWAP] THEN
      ASM_SIMP_TAC[VAL_WORD_EQ; DIMINDEX_64] THEN
      CONV_TAC WORD_REDUCE_WEAK_CONV THEN AP_THM_TAC THEN
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; ADD_CLAUSES; SUB_0] THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM_TAC BIGNUM_MODIFIER_EXEC (1--2) THEN
      ...
      ASM_REWRITE_TAC[BITVAL_CLAUSES] THEN CONV_TAC WORD_REDUCE_WEAK_CONV;
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
      ...
      CONV_TAC WORD_REDUCE_WEAK_CONV THEN REWRITE_TAC[NOT_LT];
