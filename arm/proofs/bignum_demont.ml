...
let numsteps = count_nsteps (concl BIGNUM_DEMONT_SUBROUTINE_CORRECT)
    BIGNUM_DEMONT_EXEC;;

let full_spec = mk_safety_spec
    ~numinstsopt:numsteps
    (assoc "bignum_demont" subroutine_signatures)
    BIGNUM_DEMONT_SUBROUTINE_CORRECT
    BIGNUM_DEMONT_EXEC;;

let BIGNUM_DEMONT_SUBROUTINE_SAFE = time prove
 (full_spec,
  PROVE_SAFETY_SPEC BIGNUM_DEMONT_EXEC);;
