 ...
 (`!a x:int64.
  ...
  REWRITE_TAC[ARITH_RULE `16 = 2 EXP 4`] THEN CONJ_TAC THENL
   [REWRITE_TAC[VAL_WORD; DIMINDEX_64; MOD_MOD_EXP_MIN] THEN
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN CONV_TAC MOD_DOWN_CONV THEN REFL_TAC;
  ...
  ASM_REWRITE_TAC[BITVAL_CLAUSES] THEN CONV_TAC NUM_REDUCE_WEAK_CONV);;
 ...
 (`!k z x a y b w pc.
   ...
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
     [DISCH_TAC THEN REWRITE_TAC[GSYM BIGNUM_FROM_MEMORY_BYTES] THEN
      ...
      SPEC_TAC(`16`,`e:num`) THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
  ...
  CONJ_TAC THENL
   [MAP_EVERY EXPAND_TAC ["m"; "n"] THEN REWRITE_TAC[MESON[]
    ...
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN REWRITE_TAC[MIN] THEN
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [REWRITE_TAC[MULT_CLAUSES; WORD_ADD_0; SUB_0; HIGHDIGITS_0] THEN
    ...
    (CONJ_TAC THENL [CONV_TAC WORD_REDUCE_WEAK_CONV; ALL_TAC]) THEN
  ...
  STRIP_ASSUME_TAC THENL
   [MAP_EVERY (fun n ->
      ...
      CONV_TAC(LAND_CONV(RAND_CONV(RAND_CONV num_CONV))) THEN
      ASM_REWRITE_TAC[] THEN CONV_TAC REAL_RAT_REDUCE_WEAK_CONV THEN
      DISCH_TAC THEN DISCH_TAC)
      (1--58) THEN
    REWRITE_TAC[ARITH_RULE `i <= 58 <=> i < 59`] THEN
    CONV_TAC EXPAND_CASES_CONV THEN
    ASM_REWRITE_TAC[] THEN CONV_TAC REAL_RAT_REDUCE_WEAK_CONV;
    ALL_TAC] THEN
   ...
   SUBGOAL_THEN `ODD n` ASSUME_TAC THENL
     ...
     [ASM_REWRITE_TAC[GSYM VAL_EQ_0; VAL_WORD_0; COND_SWAP] THEN
      ...
      SUBST1_TAC THENL
       [MATCH_MP_TAC CONG_IMP_EQ THEN EXISTS_TAC `64` THEN
        ...
        DISCH_THEN MATCH_MP_TAC THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
    ...
    CONJ_TAC THENL
     [ASM_CASES_TAC `ODD m_lo` THEN
      ...
      SUBST1_TAC(SYM(NUM_REDUCE_WEAK_CONV `2 EXP 1`)) THEN
   ...
   [ONCE_REWRITE_TAC[TAUT `p \/ q \/ r <=> ~p /\ ~q ==> r`] THEN
    ...
    CONJ_TAC THENL
     [ALL_TAC;
      ...
      ASM_REWRITE_TAC[REAL_NOT_LE] THEN CONV_TAC REAL_RAT_REDUCE_WEAK_CONV THEN
      REWRITE_TAC[REAL_ARITH `&0 < (x:real) * &32 <=> &0 < x`] THEN
      MATCH_MP_TAC REAL_ZPOW_LT THEN CONV_TAC REAL_RAT_REDUCE_WEAK_CONV] THEN
 ...
 (*** Congruence cross-multiplication and shift-by-6 "congloop" ***)
   ...
   (*** The final digit write ****)
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [GHOST_INTRO_TAC `cin:bool` `read CF` THEN
      ...
      REWRITE_TAC[REAL_MUL_RZERO] THEN CONV_TAC REAL_RAT_REDUCE_WEAK_CONV THEN
      SUBGOAL_THEN
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ARM_SIM_TAC CORE_MODINV_EXEC (1--5) THEN
    ...
    ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES; HIGHDIGITS_0] THEN
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN CONV_TAC REAL_RAT_REDUCE_WEAK_CONV THEN
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ARM_SIM_TAC CORE_MODINV_EXEC (1--2) THEN
    ...
    REWRITE_TAC[BITVAL_CLAUSES] THEN CONV_TAC REAL_RAT_REDUCE_WEAK_CONV;
    X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
    GHOST_INTRO_TAC `cin:bool` `read CF` THEN
    MP_TAC(GENL [`x:int64`; `a:num`]
     (ISPECL [`x:int64`; `k - i:num`; `a:num`; `i:num`]
  ...
  CONJ_TAC THENL
   [ASM_CASES_TAC `b = 0` THEN ASM_REWRITE_TAC[ODD] THEN
    ...
    ASM_REWRITE_TAC[GSYM VAL_EQ] THEN CONV_TAC WORD_REDUCE_WEAK_CONV] THEN
