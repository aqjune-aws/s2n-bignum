...
let EQUAL_FROM_CONGRUENT_REAL = prove
 (`!n x y z.
  ...
  CONV_TAC REAL_RAT_REDUCE_WEAK_CONV THEN DISCH_THEN SUBST1_TAC THEN
...
let MASK_AND_VALUE_FROM_CARRY_REAL_LT = prove
 (`!(m:int64) l k (x:real).
  ...
  CONV_TAC WORD_REDUCE_WEAK_CONV THEN CONV_TAC(RAND_CONV REAL_POLY_CONV) THEN
...
let WORD_PC_PLUS_CONV =
  let pth = prove
    (`word(pc + NUMERAL n):int64 =
     ...
     CONV_TAC NUM_REDUCE_WEAK_CONV THEN CONV_TAC MOD_DOWN_CONV THEN
     REWRITE_TAC[]) in
  let conv =
   ...
   (GEN_REWRITE_CONV ONCE_DEPTH_CONV
    ...
    GEN_REWRITE_CONV ONCE_DEPTH_CONV
     [WORD_RULE `word_sub x (word_add x y):int64 = word_neg y`;
      pth] THENC
    GEN_REWRITE_CONV ONCE_DEPTH_CONV [SYM(NUM_REDUCE_WEAK_CONV `2 EXP 64`)])) THEN
