 ...
 (`!z x n pc stackpointer.
  ...
  SUBST_ALL_TAC THENL
   [REWRITE_TAC[BITVAL_EQ_0] THEN CONV_TAC SYM_CONV THEN
    ...
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN CONV_TAC INT_ARITH;
    ALL_TAC] THEN
...
let LOCAL_ADD_P521_TAC =
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  RULE_ASSUM_TAC(REWRITE_RULE
   [SYM(NUM_REDUCE_WEAK_CONV `2 EXP 9`); GSYM WORD_OF_BITS_SING_AS_WORD;
    WORD_OF_BITS_SING_AND_WORD]) THEN
  RULE_ASSUM_TAC(REWRITE_RULE
   [WORD_OF_BITS_SING_AS_WORD; NUM_REDUCE_WEAK_CONV `2 EXP 9`]) THEN
  ...
  SUBST_ALL_TAC THENL
   [COND_CASES_TAC THEN ASM_REWRITE_TAC[] THEN
    CONV_TAC WORD_REDUCE_WEAK_CONV THEN CONV_TAC NUM_REDUCE_WEAK_CONV;
    ALL_TAC] THEN
...
let GENERAL_SUB_P521_TAC localvars =
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_REWRITE_TAC[SYM(NUM_REDUCE_WEAK_CONV `2 EXP 9 - 1`)] THEN
  MATCH_MP_TAC EQUAL_FROM_CONGRUENT_REAL THEN
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
   ...
   [MATCH_MP_TAC(ARITH_RULE `n + m:num = p ==> p - n = m`) THEN
    ...
    SUBST1_TAC THENL
     [REWRITE_TAC[REAL_VAL_WORD_XOR];
      ...
      REAL_ARITH_TAC] THEN
    CONV_TAC WORD_REDUCE_WEAK_CONV THEN CONV_TAC REAL_RAT_REDUCE_WEAK_CONV THEN
  ...
  SUBST_ALL_TAC THENL
   [EXPAND_TAC "d" THEN ONCE_REWRITE_TAC[WORD_BITWISE_RULE
    ...
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN CONV_TAC WORD_REDUCE_WEAK_CONV;
    ALL_TAC] THEN
  ...
  (LABEL_TAC "*") THENL
   [CONV_TAC SYM_CONV THEN EXPAND_TAC "ca" THEN
    ...
    REWRITE_TAC[SYM(NUM_REDUCE_WEAK_CONV `64 * 8`)] THEN
  ...
  MP_TAC THENL
   [REMOVE_THEN "*" (SUBST1_TAC o SYM) THEN EXPAND_TAC "carry_s80" THEN
    ...
    REWRITE_TAC[bignum_of_wordlist] THEN CONV_TAC WORD_REDUCE_WEAK_CONV THEN
  ...
  REWRITE_TAC[SYM(NUM_REDUCE_WEAK_CONV `2 EXP 9 - 1`)] THEN
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  SUBGOAL_THEN `4 * m = m'` ASSUME_TAC THENL
   [SUBGOAL_THEN `m DIV 2 EXP 512 < 2 EXP 62` MP_TAC THENL
    ...
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN DISCH_TAC THEN DISCH_TAC THEN
    ...
    CONV_TAC WORD_REDUCE_WEAK_CONV THEN REWRITE_TAC[BITVAL_CLAUSES] THEN
    CONV_TAC NUM_RING;
    ALL_TAC] THEN
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  SUBGOAL_THEN `8 * (p_521 - n) = n'` ASSUME_TAC THENL
   [MATCH_MP_TAC(ARITH_RULE `8 * n + m = 8 * p ==> 8 * (p - n) = m`) THEN
    ...
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN STRIP_TAC THEN
    REWRITE_TAC[bignum_of_wordlist; REAL_OF_NUM_CLAUSES] THEN
    REWRITE_TAC[val_def; DIMINDEX_64; bignum_of_wordlist] THEN
    REWRITE_TAC[ARITH_RULE `i < 64 <=> 0 <= i /\ i <= 63`] THEN
    REWRITE_TAC[GSYM IN_NUMSEG; IN_GSPEC] THEN
...
let weierstrass_of_jacobian_p521_double = prove
 (`!P1 P2 x1 y1 z1 x3 y3 z3.
  ...
  REWRITE_TAC[p_521; b_521] THEN CONV_TAC INT_REDUCE_WEAK_CONV);;
 ...
 (`!p3 p1 t1 pc stackpointer.
  ...
  REPEAT(ANTS_TAC THENL
   [REWRITE_TAC[p_521] THEN RULE_ASSUM_TAC(REWRITE_RULE[p_521]) THEN
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN ASM BOUNDER_TAC[];
...
let LOCAL_JADD_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
  ...
  REWRITE_TAC[GSYM(NUM_REDUCE_WEAK_CONV `2 EXP 256`)] THENL
   [ASM_REWRITE_TAC[] THEN MAP_EVERY EXPAND_TAC ["P1"; "P2"] THEN
    ...
    REWRITE_TAC[p_521] THEN CONV_TAC INT_REDUCE_WEAK_CONV;
  ...
  ASM_REWRITE_TAC[jacobian_add_unexceptional; nistp521;
                  INTEGER_MOD_RING_CLAUSES] THEN
  REWRITE_TAC[p_521] THEN CONV_TAC INT_REDUCE_WEAK_CONV THEN
  ...
  ANTS_TAC THENL
   [REWRITE_TAC[FIELD_INTEGER_MOD_RING; PRIME_P521] THEN
    REWRITE_TAC[b_521; IN_INTEGER_MOD_RING_CARRIER; p_521;
                INTEGER_MOD_RING_CLAUSES] THEN
    CONV_TAC INT_REDUCE_WEAK_CONV THEN REWRITE_TAC[GSYM p_521] THEN
    CONJ_TAC THENL [ALL_TAC; CONJ_TAC] THEN
    MATCH_MP_TAC MODULAR_DECODE THEN
    REWRITE_TAC[p_521] THEN CONV_TAC NUM_REDUCE_WEAK_CONV;
...
let P521_JSCALARMUL_CORRECT = time prove
 (`!res scalar point n xyz pc stackpointer.
  ...
  CONV_TAC(LAND_CONV(LAND_CONV NUM_REDUCE_WEAK_CONV)) THEN DISCH_TAC THEN
  ...
  REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    ASM_SIMP_TAC[GSYM GROUP_POW_ADD] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
   ...
   [EXPAND_TAC "bf" THEN
    ...
    (CONJUNCTS_THEN SUBST1_TAC) THENL
     [CONV_TAC(BINOP_CONV SYM_CONV) THEN MATCH_MP_TAC DIVMOD_UNIQ THEN
      ASM_REWRITE_TAC[] THEN CONJ_TAC THENL
       [EXPAND_TAC "bf" THEN REWRITE_TAC[GSYM(NUM_REDUCE_WEAK_CONV `2 EXP 5`)] THEN
  ...
  ASSUME_TAC THENL
   [X_GEN_TAC `P:(int#int)option` THEN STRIP_TAC THEN
    ...
    CONV_TAC WORD_REDUCE_WEAK_CONV THEN
    MATCH_MP_TAC CONG_IMP_EQ THEN EXISTS_TAC `2 EXP 521` THEN CONJ_TAC THENL
    ...
    CONJ_TAC THENL
     [SUBGOAL_THEN
      ...
      CONV_TAC(ONCE_DEPTH_CONV BIGNUM_OF_WORDLIST_DIV_CONV) THEN
      REWRITE_TAC[p_521] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
      CONV_TAC WORD_BLAST;
      ALL_TAC] THEN
