...
let REPRESENTS_P256_NEGATION = prove
 (`!P x y z.
  ...
  ASM_CASES_TAC `Y:int = &0` THENL
   [ASM_REWRITE_TAC[] THEN CONV_TAC INT_REDUCE_WEAK_CONV THEN
  ...
  ASM_CASES_TAC `y = 0` THENL
   [ASM_REWRITE_TAC[ring_div; RING_INV_INTEGER_MOD_RING; INT_OF_NUM_CLAUSES;
    ...
    REWRITE_TAC[MULT_CLAUSES; p_256] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
 ...
 (`!res scalar blocksize tab n len tabulation pc stackpointer.
   ...
   [ARITH_TAC;
     ...
     [ASM_REWRITE_TAC[VAL_WORD; VAL_WORD_MUL; ADD_CLAUSES; DIMINDEX_64] THEN
      ...
      ANTS_TAC THENL
       [TRANS_TAC LET_TRANS `31 * (256 + 1)` THEN
        CONJ_TAC THENL [MATCH_MP_TAC LE_MULT2; CONV_TAC NUM_REDUCE_WEAK_CONV] THEN
    ...
    REPEAT(ANTS_TAC THENL
     [TRY(COND_CASES_TAC THEN REWRITE_TAC[MULT_CLAUSES; LE_0] THEN
          ...
          REWRITE_TAC[p_256] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN STRIP_TAC) THEN
  ...
  THEN REPEAT CONJ_TAC THENL
   [REWRITE_TAC[EXP_EQ_0; ARITH_EQ];
    ...
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN CONV_TAC EXPAND_CASES_CONV THEN
    CONV_TAC(DEPTH_CONV WORD_NUM_RED_CONV) THEN
   ...
   [POP_ASSUM MP_TAC THEN MATCH_MP_TAC(TAUT `~p ==> p ==> q`) THEN
    ASM_CASES_TAC `&n rem &2 pow (blocksize * i) = &0` THENL
     [FIRST_X_ASSUM(K ALL_TAC o check (is_imp o concl)) THEN
      ...
      CONV_TAC NUM_REDUCE_WEAK_CONV THEN CONV_TAC INT_REDUCE_WEAK_CONV;
      REWRITE_TAC[ARITH_RULE `b * (i + 1) = b * i + b`; INT_POW_ADD] THEN
  ...
  THENL
   [SUBGOAL_THEN `&bf:int = &0` SUBST_ALL_TAC THENL
    ...
    REWRITE_TAC[n_256] THEN CONV_TAC NUM_REDUCE_WEAK_CONV;
    ALL_TAC] THEN
  ...
  REWRITE_TAC[n_256] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
  REWRITE_TAC[GSYM(NUM_REDUCE_WEAK_CONV `2 EXP 256`)] THEN
  ...
  CONJ_TAC THENL [CONV_TAC NUM_REDUCE_WEAK_CONV; ALL_TAC] THEN
