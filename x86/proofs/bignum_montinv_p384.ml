...
let lemma1,lemma2 = (CONJ_PAIR o prove)
 (`(--(&2 pow 20) <= h /\ h < &2 pow 20 /\ &0 <= l /\ l < &2 pow 43
  ...
  REWRITE_TAC[DIMINDEX_64] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
  ASM_INT_ARITH_TAC);;
...
let DIVSTEP_FVEC_ODD = prove
 (`!n d f g.
  ...
  REWRITE_TAC[INT_REM_REM_POW_MIN] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
...
let DIVSTEP_FGVEC_PACK = prove
 (`!d f g. word_or (word_and (iword f) (word 1048575))
  ...
  CONV_TAC INT_REDUCE_WEAK_CONV THEN REWRITE_TAC[IWORD_INT_ADD] THEN
  CONV_TAC WORD_REDUCE_WEAK_CONV THEN AP_THM_TAC THEN AP_TERM_TAC THEN
  REWRITE_TAC[WORD_AND_MASK_WORD; GSYM(NUM_REDUCE_WEAK_CONV `2 EXP 20 - 1`)] THEN
  ...
  CONV_TAC INT_REDUCE_WEAK_CONV THEN REWRITE_TAC[INT_REM_EQ] THEN
  MATCH_MP_TAC(INTEGER_RULE
   `!m:int. (x == y) (mod m) /\ n divides m ==> (x == y) (mod n)`) THEN
  EXISTS_TAC `(&2:int) pow dimindex(:64)` THEN REWRITE_TAC[VAL_IWORD_CONG] THEN
  REWRITE_TAC[DIMINDEX_64; GSYM INT_REM_EQ_0] THEN CONV_TAC INT_REDUCE_WEAK_CONV);;
...
let PACKED_DIVSTEP_TAC n shf s =
  ...
  MP_TAC THENL
   [ASM_REWRITE_TAC[WORD_AND_1_BITVAL] THEN
    ...
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN INT_ARITH_TAC;
...
let DIVSTEP_MAT_UNPACK_20 = prove
 (`word_ishr
  ...
  REWRITE_TAC[divstep_fvec; divstep_gvec] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
  REWRITE_TAC[INT_ARITH
  ...
  CONV_TAC INT_REDUCE_WEAK_CONV THEN INT_ARITH_TAC);;
...
let LOCAL_WORD_DIVSTEP59_CORRECT = prove
 (`!d f g stackpointer pc.
  ...
  ASM_REWRITE_TAC[] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
...
let PRIME_P384 = time prove
 (`prime p_384`,
  REWRITE_TAC[p_384] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
...
let IWORD_CASES_LEMMA = prove
 (`!m:int64.
  ...
  REWRITE_TAC[DIMINDEX_64; GSYM MSB_IVAL] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
...
let TWOS_COMPLEMENT_448 = prove
 (`(&(bignum_of_wordlist[n0;n1;n2;n3;n4;n5;n6]) == x) (mod (&2 pow 448))
  ...
  CONV_TAC NUM_REDUCE_WEAK_CONV THEN ANTS_TAC THENL [BOUNDER_TAC[]; ALL_TAC] THEN
...
let TWOS_COMPLEMENT_448_WEAK = prove
 (`(&(bignum_of_wordlist[n0;n1;n2;n3;n4;n5;n6]) == x) (mod (&2 pow 448))
  ...
  ASM_REWRITE_TAC[ARITH_EQ; NUM_REDUCE_WEAK_CONV `448 - 1`] THEN
...
let mmlemma_alt = prove
 (`!h (l:int64) (x:int64).
  ...
  REWRITE_TAC[CONG] THEN CONV_TAC NUM_REDUCE_WEAK_CONV);;
 ...
 (`!z x n pc stackpointer.
  ...
  THEN REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    DISCARD_STATE_TAC "s56" THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
    ...
    ONCE_REWRITE_TAC[GSYM INT_MUL_REM] THEN CONV_TAC INT_REDUCE_WEAK_CONV THEN
    ...
    REPEAT CONJ_TAC THENL
     [CONV_TAC NUM_REDUCE_WEAK_CONV THEN BOUNDER_TAC[];
    ...
    SUBGOAL_THEN `carry_s26 <=> n < p_384` (SUBST1_TAC o SYM) THENL
     [MATCH_MP_TAC FLAG_FROM_CARRY_LT THEN EXISTS_TAC `384` THEN
      ...
      COND_CASES_TAC THEN ASM_REWRITE_TAC[BITVAL_CLAUSES] THEN
      CONV_TAC NUM_REDUCE_WEAK_CONV THEN CONV_TAC WORD_REDUCE_WEAK_CONV THEN
    ...
    (STRIP_ASSUME_TAC o CONV_RULE NUM_REDUCE_WEAK_CONV) THENL
    ...
    ANTS_TAC THENL
     [EXPAND_TAC "t" THEN MATCH_MP_TAC DIVSTEP_F_ODD THEN
      REWRITE_TAC[p_384] THEN CONV_TAC INT_REDUCE_WEAK_CONV;
    ...
    GEN_REWRITE_TAC I [CONJ_ASSOC] THEN CONJ_TAC THENL
     [REWRITE_TAC[INT_POW_ADD; INT_ARITH
      ...
      REPEAT(FIRST_X_ASSUM(SUBST1_TAC o MATCH_MP
       (MESON[INT_DIV_MUL; INT_POW_EQ_0; INT_REDUCE_WEAK_CONV `&2:int = &0`]
          ...
          STRIP_ASSUME_TAC THENL
           [EXPAND_TAC "t" THEN CONJ_TAC THENL
             [MATCH_MP_TAC DIVSTEP_F_BOUNDS;
              MATCH_MP_TAC DIVSTEP_G_BOUNDS] THEN
            REWRITE_TAC[p_384] THEN CONV_TAC INT_REDUCE_WEAK_CONV THEN
      ...
      REWRITE_TAC[REAL_CONGRUENCE] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
    ...
    THENL
     [REPEAT GEN_TAC THEN MATCH_MP_TAC(INTEGER_RULE
      ...
      REWRITE_TAC[COPRIME_2; p_384] THEN CONV_TAC NUM_REDUCE_WEAK_CONV;
      ALL_TAC] THEN
    SUBGOAL_THEN
  ...
  ANTS_TAC THENL
   [REWRITE_TAC[VAL_BOUND_64] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
  ...
  REPEAT CONJ_TAC THENL
   [REWRITE_TAC[INT_OF_NUM_CLAUSES; GSYM num_coprime] THEN
    REWRITE_TAC[COPRIME_LEXP; COPRIME_2; p_384] THEN CONV_TAC NUM_REDUCE_WEAK_CONV;
    EXPAND_TAC "fsgn" THEN REWRITE_TAC[bitval] THEN
    COND_CASES_TAC THEN ASM_REWRITE_TAC[] THEN CONV_TAC INT_REDUCE_WEAK_CONV;
    ALL_TAC] THEN
  SUBGOAL_THEN
   `&2 pow 64 * fsgn:int =
  ...
  SUBST1_TAC THENL
   [SUBST1_TAC(SYM(ASSUME `--((mm:int^2^2)$1$1) = m00`)) THEN
    ...
    CONV_TAC INT_REDUCE_WEAK_CONV THEN REWRITE_TAC[BITVAL_CLAUSES] THEN
    INT_ARITH_TAC;
    ALL_TAC] THEN
  MATCH_MP_TAC(INTEGER_RULE
  ...
  REWRITE_TAC[GSYM INT_POW_ADD] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
  RULE_ASSUM_TAC(REWRITE_RULE[NUM_REDUCE_WEAK_CONV `843 - 5 * 14`]) THEN
  ASM_REWRITE_TAC[]);;
