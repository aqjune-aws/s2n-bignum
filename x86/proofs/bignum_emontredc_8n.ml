 ...
 (`!k z m w a n pc stackpointer.
  ...
  ASM_CASES_TAC `k8 = 0` THENL
   [UNDISCH_THEN `k8 = 0` SUBST_ALL_TAC THEN
    SUBGOAL_THEN `val(word_ushr(word k:int64) 3) = 0` ASSUME_TAC THENL
     [ASM_REWRITE_TAC[VAL_WORD_USHR; NUM_REDUCE_WEAK_CONV `2 EXP 3`];
      ALL_TAC] THEN
  ...
  CONJ_TAC THENL
   [X86_SIM_TAC BIGNUM_EMONTREDC_8N_EXEC (1--3) THEN
    ...
    ASM_REWRITE_TAC[word_ushr; NUM_REDUCE_WEAK_CONV `2 EXP 3`] THEN
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    CONV_TAC(LAND_CONV(LAND_CONV WORD_REDUCE_WEAK_CONV)) THEN CONV_TAC WORD_RULE;
  ...
  CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
    ...
    REWRITE_TAC[GSYM BIGNUM_FROM_MEMORY_BYTES] THEN
    REWRITE_TAC[NUM_REDUCE_WEAK_CONV `8 * 8`] THEN
    REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
    DISCH_TAC THEN
    SUBGOAL_THEN
     `(!i. i < 8
