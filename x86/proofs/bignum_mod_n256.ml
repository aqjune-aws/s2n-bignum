  ...
  ASM_CASES_TAC `k < 4` THENL
   [SUBGOAL_THEN `n MOD n_256 = n` SUBST1_TAC THENL
     [MATCH_MP_TAC MOD_LT THEN
      ...
      ASM_REWRITE_TAC[LE_EXP; n_256] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
      ASM_ARITH_TAC;
      ALL_TAC] THEN
   ...
   [ABBREV_TAC `j = k - 4` THEN VAL_INT64_TAC `j:num` THEN
    ...
    ANTS_TAC THENL
     [TRANS_TAC LTE_TRANS `2 EXP (64 * 4)` THEN
      ASM_REWRITE_TAC[n_256] THEN CONV_TAC NUM_REDUCE_WEAK_CONV;
    ...
    CONJ_TAC THENL [BOUNDER_TAC[]; ALL_TAC] THEN CONJ_TAC THENL
     [UNDISCH_TAC `m < 2 EXP (64 * 4)` THEN REWRITE_TAC[n_256] THEN
      POP_ASSUM_LIST(K ALL_TAC) THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
