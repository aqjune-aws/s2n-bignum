...
let LOCAL_MUL_5_TAC =
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  SUBGOAL_THEN `m * n < 2 EXP 512` ASSUME_TAC THENL
   [MAP_EVERY EXPAND_TAC ["m"; "n"] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
    BOUNDER_TAC[];
    ALL_TAC] THEN
...
let LOCAL_SUB_4_TAC =
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  REWRITE_TAC[SYM(NUM_REDUCE_WEAK_CONV `2 EXP 63`); VAL_WORD_AND_POW2] THEN
...
let nintlemma = prove
 (`&(num_of_int(x rem &p_25519)) = x rem &p_25519`,
  ...
  REWRITE_TAC[INT_OF_NUM_EQ; p_25519] THEN CONV_TAC NUM_REDUCE_WEAK_CONV);;

let lemma = prove
 (`X = num_of_int(x rem &p_25519) ==> X < p_25519 /\ &X = x rem &p_25519`,
  DISCH_THEN SUBST1_TAC THEN
  REWRITE_TAC[GSYM INT_OF_NUM_LT; nintlemma; INT_LT_REM_EQ] THEN
  REWRITE_TAC[INT_OF_NUM_LT; p_25519] THEN CONV_TAC NUM_REDUCE_WEAK_CONV);;
 ...
 (`!res scalar n point X pc stackpointer.
  ...
  REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    SIMP_TAC[FIELD_NONTRIVIAL] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
    REWRITE_TAC[COND_ID];
  ...
  ASSUME_TAC THENL
   [EXPAND_TAC "nn" THEN REWRITE_TAC[GSYM BIGNUM_FROM_MEMORY_BYTES] THEN
    ...
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN REFL_TAC;
    ALL_TAC] THEN
  ...
  ASM_REWRITE_TAC[] THEN COND_CASES_TAC THEN ASM_REWRITE_TAC[] THENL
   [FIRST_X_ASSUM SUBST_ALL_TAC THEN
    ...
    REPEAT(ANTS_TAC THENL
     [ASM_REWRITE_TAC[] THEN
      REWRITE_TAC[p_25519] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
      RULE_ASSUM_TAC(CONV_RULE NUM_REDUCE_WEAK_CONV o REWRITE_RULE[p_25519]) THEN
      ASM BOUNDER_TAC[];
      STRIP_TAC]) THEN
