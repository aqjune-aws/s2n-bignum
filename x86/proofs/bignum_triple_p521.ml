...
let overflowbit8lemma = prove
 (`!n:int64.
  ...
  REWRITE_TAC[IVAL_VAL; DIMINDEX_64] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
  ...
  REWRITE_TAC[BIT_WORD_SHL; DIMINDEX_64] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
...
let BIGNUM_TRIPLE_P521_CORRECT = time prove
 (`!z x n pc.
  ...
  SUBST_ALL_TAC THENL
   [REWRITE_TAC[GSYM(NUM_REDUCE_WEAK_CONV `2 EXP 9`); VAL_WORD_AND_POW2] THEN
