...
let CORE_INV_P521_CORRECT = time prove
 (`!z x n pc stackpointer.
  ...
  THEN REPEAT CONJ_TAC THENL
   [ARITH_TAC;
    ...
    DISCARD_STATE_TAC "s77" THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
    ...
    REWRITE_TAC[SYM(NUM_REDUCE_WEAK_CONV `2 EXP 9 - 1`)] THEN EXPAND_TAC "n" THEN
    REWRITE_TAC[VAL_WORD_AND_MASK_WORD; bignum_of_wordlist] THEN
    REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES] THEN
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN REWRITE_TAC[REAL_OF_NUM_MOD; p_521] THEN
    ACCUMULATOR_POP_ASSUM_LIST(MP_TAC o end_itlist CONJ o DESUM_RULE) THEN
    COND_CASES_TAC THEN ASM_REWRITE_TAC[BITVAL_CLAUSES] THEN
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN REAL_INTEGER_TAC;
    ...
    (*** The two actual modular reductions ***)
     ...
     [ABBREV_TAC `qu:int64 =
      ...
      REWRITE_TAC[DIMINDEX_64; p_521] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
 ...
 CONV_TAC SYM_CONV THEN REWRITE_TAC[GSYM INT_OF_NUM_REM] THEN
  ...
  REWRITE_TAC[INT_OF_NUM_CLAUSES; COPRIME_LEXP; COPRIME_2; GSYM num_coprime;
              p_521] THEN
  CONV_TAC NUM_REDUCE_WEAK_CONV THEN EXPAND_TAC "fsgn" THEN
  FIRST_X_ASSUM(SUBST1_TAC o MATCH_MP (INT_ARITH
   `abs(x:int) = &1 ==> x = if x < &0 then -- &1 else &1`)) THEN
  COND_CASES_TAC THEN CONV_TAC INT_REDUCE_WEAK_CONV THEN
