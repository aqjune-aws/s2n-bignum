...
(* The Windows .obj's .text contains a relocation against .rodata, so it is
   ...
   The .rodata content is identical to the Linux ABI build, so the
   WHOLE_READONLY_data constant is reused. *)
...
(* The Windows trimmed mc has length 0x3184: a 16-byte ABI-translation wrapper
   (PUSH RDI/RSI; MOV RDI<-RCX; MOV RSI<-RDX; CALL +3; POP RSI/RDI; RET) at
   offsets 0..15, then the Linux ABI trimmed body of length 0x3174 starting at
   pc+0x10. The LEA targeting `tables` is at the same instruction as in the
   Linux ABI body, so the riprel32_within_bounds bound is `tables (pc + 190)`
   for the trimmed mc (since `(pc + 0x10) + 174 = pc + 190`) and
   `tables (pc + 194)` for the IBT mc. *)
