   ...
   [GHOST_INTRO_TAC `n:num` `bignum_from_memory (t,k)` THEN
    ...
    CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--2) THEN
      ...
      CONV_TAC WORD_REDUCE_WEAK_CONV THEN ONCE_REWRITE_TAC[WORD_XOR_SYM] THEN
      ...
      CONV_TAC NUM_REDUCE_WEAK_CONV THEN REWRITE_TAC[WORD_SUB] THEN
      ...
      ASM_SIMP_TAC[VAL_WORD_EQ; DIMINDEX_64] THEN COND_CASES_TAC THENL
       [ASM_SIMP_TAC[ARITH_RULE `r <= h ==> (h < r + 1 <=> h = r)`] THEN
        ...
        ASM_CASES_TAC `i = 0` THEN ASM_REWRITE_TAC[] THENL
         [CONV_TAC NUM_REDUCE_WEAK_CONV; DISCH_THEN(MP_TAC o AP_TERM `EVEN`)] THEN
        ASM_REWRITE_TAC[EVEN_EXP] THEN CONV_TAC NUM_REDUCE_WEAK_CONV]];
    ALL_TAC] THEN
   ...
   [GHOST_INTRO_TAC `rhi:num` `\s. val(read RCX s)` THEN
    ...
    ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
     [X86_SIM_TAC BIGNUM_MONTIFIER_EXEC (1--6) THEN
      ...
      CONV_TAC REAL_RAT_REDUCE_WEAK_CONV THEN REWRITE_TAC[WORD_NOT_MASK] THEN
