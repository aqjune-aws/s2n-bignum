...
let BIGNUM_OPTSUBADD_CORRECT = prove
 (`!k z x p y a b pc.
   ...
   [ASM_REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
    ...
    CONJ_TAC THENL
     [MATCH_MP_TAC(INT_RING `x * y:int = z ==> a + x * y * b = a + z * b`) THEN
      ...
      CONV_TAC WORD_REDUCE_WEAK_CONV THEN CONV_TAC INT_REDUCE_WEAK_CONV THEN
    ...
    SUBST1_TAC THENL
     [SUBST1_TAC(INT_ARITH
      ...
      CONV_TAC INT_REDUCE_WEAK_CONV THEN CONV_TAC WORD_REDUCE_WEAK_CONV]] THEN
  ...
  CONV_TAC WORD_REDUCE_WEAK_CONV THEN INT_ARITH_TAC);;
