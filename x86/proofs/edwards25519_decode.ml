 ...
 (`!z x y m n pc.
  ...
  SUBST_ALL_TAC THENL
   [REWRITE_TAC[VAL_WORD_SHL; DIMINDEX_64] THEN
    ...
    CONV_TAC WORD_REDUCE_WEAK_CONV THEN REWRITE_TAC[ODD_MULT; ARITH];
  ...
  ASM_SIMP_TAC[SYM(NUM_REDUCE_WEAK_CONV `2 EXP 63 - 1`)] THEN
  ...
  CONV_TAC WORD_REDUCE_WEAK_CONV THEN REAL_INTEGER_TAC);;
 ...
 (`!z k x n pc stackpointer.
  ...
  REPEAT CONJ_TAC THENL
   [ASM_SIMP_TAC[LE_1];
    ...
    REWRITE_TAC[EXP; EXP_1; CONG_REFL; SUB_0] THEN CONV_TAC NUM_REDUCE_WEAK_CONV;
...
let EDWARDS25519_NONZERO_DENOMINATORS = prove
 (`!y. coprime(&1 + &d_25519 * y pow 2,&p_25519)`,
  ...
  REWRITE_TAC[] THEN
  ONCE_REWRITE_TAC[GSYM INT_CONG_RREM] THEN CONV_TAC INT_REDUCE_WEAK_CONV THEN
  REWRITE_TAC[INT_OF_NUM_CLAUSES; GSYM num_congruent] THEN
...
let EXISTS_IN_GROUP_CARRIER_EDWARDS25519 = prove
 (`!y. (?x. (x,&y) IN group_carrier edwards25519_group) <=>
  ...
  SUBGOAL_THEN `&p_25519 - &1:int = &(p_25519 - 1)` SUBST1_TAC THENL
   [REWRITE_TAC[p_25519] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN ARITH_TAC;
...
let EDWARDS25519_DECODE_CORRECT = time prove
 (`!z c n pc stackpointer.
  ...
  RULE_ASSUM_TAC(REWRITE_RULE[NUM_REDUCE_WEAK_CONV `64 * 4`]) THEN
  ...
  ASSUME_TAC THENL
   [CONV_TAC(LAND_CONV BIGNUM_LEXPAND_CONV) THEN ASM_REWRITE_TAC[] THEN
    ...
    REWRITE_TAC[p_25519] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
    REWRITE_TAC[bignum_of_wordlist; GSYM REAL_OF_NUM_CLAUSES] THEN
    REWRITE_TAC[p_25519] THEN CONV_TAC REAL_RAT_REDUCE_WEAK_CONV THEN
    ACCUMULATOR_ASSUM_LIST(MP_TAC o end_itlist CONJ o DESUM_RULE) THEN
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN REAL_INTEGER_TAC;
  ...
  REWRITE_TAC[SYM(NUM_REDUCE_WEAK_CONV `2 EXP 252 - 3`)] THEN DISCH_TAC THEN
  ...
  SUBST1_TAC THENL
   [REWRITE_TAC[WORD_AND_1_BITVAL; BIT_LSB; MOD_2_CASES] THEN
    ...
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN UNDISCH_TAC `b < 2` THEN
  ...
  ANTS_TAC THENL
   [EXPAND_TAC "e" THEN REWRITE_TAC[CONG] THEN
    ...
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN MATCH_MP_TAC CONG_EXP THEN
   ...
   [REWRITE_TAC[IN_GROUP_CARRIER_EDWARDS25519; LET_DEF; LET_END_DEF] THEN
    ...
    SUBGOAL_THEN `(v * x EXP 2 == u) (mod p_25519)` MP_TAC THENL
     [FIRST_X_ASSUM(MP_TAC o check (is_disj o concl));
      ...
      REWRITE_TAC[p_25519] THEN CONV_TAC NUM_REDUCE_WEAK_CONV THEN
      CONV_TAC INT_REDUCE_WEAK_CONV THEN REWRITE_TAC[INT_DIVIDES_REFL]] THEN
    ...
    EXPAND_TAC "x" THEN COND_CASES_TAC THEN ASM_REWRITE_TAC[DISJ_ASSOC] THENL
     [EXPAND_TAC "s" THEN MATCH_MP_TAC CONG_TRANS THEN
      ...
      REWRITE_TAC[CONG; p_25519; j_25519] THEN CONV_TAC NUM_REDUCE_WEAK_CONV];
    ALL_TAC] THEN
  ...
  ASM_REWRITE_TAC[WORD_BITVAL_EQ_0; WORD_BITVAL_EQ_1] THENL
   [REWRITE_TAC[TAUT `~p /\ (p ==> q) <=> ~p`] THEN
    ...
    CONV_TAC NUM_REDUCE_WEAK_CONV THEN REWRITE_TAC[MULT_CLAUSES] THEN
    ...
    CONJ_TAC THENL
     [REWRITE_TAC[GSYM EXP_2] THEN DISCH_THEN(MP_TAC o MATCH_MP(NUMBER_RULE
      ...
      SIMP_TAC[] THEN CONV_TAC NUM_REDUCE_WEAK_CONV];
